...
void MyOctant::Init(void)
{
	...
	float fSizeY = glm::distance(vector3(0, v3MinG.y, 0), vector3(0, v3MinG.y, 0));
	float fSizeZ = glm::distance(vector3(0, 0, v3MinG.z), vector3(0, 0, v3MinG.z));
...
void MyOctant::Subdivide(void) {
	...
	for (uint index = 0; index < 8; index++) {
		m_pChildren[index].m_fSize = this->m_fSize;
